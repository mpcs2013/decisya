...
| NFR-13 | Maintainability | ≥ 95% line coverage on `Decisya.SharedKernel`'s `TenantId`, `Result`, `Result<T>` and `Error` types | Microsoft.Testing.Platform code coverage (`Microsoft.Testing.Extensions.CodeCoverage`, `dotnet test --coverage`) report (0.04b) |
