...
| Tool                                                     | VS 2026                                                                                                                                                                          | CLI                                                                                                                                          |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| .NET 10 SDK (LTS)                                        | Stable **Visual Studio 2026** Community; tick the *.NET desktop/web* workloads. No Insiders channel needed                                                                        | Download 10.0.1xx from dotnet.microsoft.com/download/dotnet/10.0; `dotnet --version` must print `10.0.1…` (`global.json` pins 10.0.100, `rollForward: latestFeature`) |
...
If a template name above does not match what your SDK offers, run `dotnet new list aspire` and use the listed short name; report the exact output if it fails rather than guessing. Aspire 13.5 templates target .NET 10 by default, so no framework override is needed; `Directory.Build.props` sets `net10.0` for every project regardless.

Every `Aspire.*` package must be 13.5.0; mixing 13.4.x and 13.5.x fails at runtime with `TypeLoadException`. Record any package without a net10 build in the table in `docs/adr/0009-dotnet-10-lts-baseline.md`.
