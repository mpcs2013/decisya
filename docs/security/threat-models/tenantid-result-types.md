...
- Scope: what #32 adds to `Decisya.SharedKernel`: `TenantId`, `TenantIdFormatException`, the internal `TenantIdJsonConverter`, `DomainError`, `ErrorCategory`, `Result` and `Result<T>`. No endpoint, no data store, no message, no package.
...
- Rename (Marco, 2026-09-26): the result error type is `DomainError`, not G2's `Error`, because `Error` triggers CA1716 (a VB keyword clash, a build error at this repo's analyzer level) and no suppression is added. `ErrorCategory` and the `Result.Error` / `Result<T>.Error` property keep their names.
...
The G2 design settles the rest, and G3 confirms it: the `D`-only format, the 128-byte JSON pre-check, no input in `TenantIdFormatException`, `[JsonIgnore]` on `DomainError.Message`, the `DomainError.Code` pattern, `Write` throwing for the default `TenantId`, and no `IParsable`. Each one is pinned by a requirement below, so G6 can check it.
...
flowchart LR
  ...
  subgraph sk["Decisya.SharedKernel (#32)"]
    ...
    res["Result / Result&lt;T&gt; / DomainError"]
  end
...
| T-09 | `Result<T>.Value` on failure, `Result<T>.Error` on success | I, E | Reading `Value` on a failure yields `default(T)`: a zero balance, an empty list or a default `TenantId` flows on as valid data. Alternatively, the thrown exception's message carries `DomainError.Message` or `Value`. | Low | Both getters throw `InvalidOperationException` with a fixed message that contains neither (G4-32-09). `ToString()` never renders `Value` or `Message` (G4-32-09). | V15.3, V16.5 | Mitigated by requirements |
...
- **G4-32-08 (`DomainError` safety).** As in G2:
...
- **G4-32-09 (Result accessors).** `Result<T>.Value` on a failure, and `Result<T>.Error` on a success, throw `InvalidOperationException` with a fixed message. Test: with a canary in `DomainError.Message`, and a `Value` whose `ToString()` returns a canary, neither canary appears in the exception's `Message` or `ToString()`, or in `Result<T>.ToString()`.
- **G4-32-10 (no null success).** `Result.Success<T>(null!)` and the implicit conversion from a null `T` (for example `string? s = null; Result<string> r = s!;`) throw `ArgumentNullException`. `Result.Failure(null!)`, `Result.Failure<T>(null!)` and the implicit conversion from a null `DomainError` throw `ArgumentNullException`. Test: one fact each.
...
- **G4-32-14 (`DomainError.Message` rule).** The XML doc on `DomainError.Message` reads: "developer-facing detail for the structured log; never raw user input, PII, tokens, secrets or row values; never sent to a client." The doc on `Result` says: "authorization and tenant checks must not be expressed as a `Result` a caller can ignore." Test: G6 check.
...
- `DomainError.Message` content is constrained by documentation and G6 review, not by code (T-06).
