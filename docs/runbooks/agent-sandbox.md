...
Every host here is justified by a deny-log line or a failing command, recorded in `docs/ai/pipeline/36.md`'s G4 evidence, and approved by Marco — never add one yourself; ask first (same rule as the note under [Reviewing the egress log](#reviewing-the-egress-log) below). `claude.ai`, `console.anthropic.com` and `downloads.claude.ai` are deliberately **not** allow-listed, which removes the *browser-side* half of an interactive login and the auto-update path. It does **not** remove the whole login path: `platform.claude.com` (needed for interactive startup's own connectivity check, above) also serves Claude Code's OAuth token, authorize and code-callback endpoints, so a claude.ai subscription `/login` can still complete from inside the sandbox (N-01, `docs/security/reviews/36.md`). **Never run `/login` inside this sandbox — the dedicated API key is the only credential it uses.** `sandbox.py claude` and `sandbox.py shell` both refuse to start if `~/.claude/.credentials.json` already exists inside `workspace`, and warn if it appears after a session; see [Troubleshooting](#troubleshooting).
...
| # | Visual Studio 2026 | CLI |
| --- | --- | --- |
| 1 | Before the **first** use of `--with-docker`: update Docker Desktop to the current version and run `wsl --update` at least once (36:G4-12(e); `sandbox.py` also refuses below a recorded Docker Engine version floor, G4-41-10) — *Docker Desktop → Settings → Software updates*, *PowerShell:* `wsl --update` | `wsl --update`; update Docker Desktop from its own updater |
...
`up --with-docker` builds a **fresh** engine every time (G4-41-04): it removes and recreates the `docker` sidecar and both of its volumes before pre-loading the image allow-list, so no agent session ever runs against a store an earlier session could have written to. It refuses while a `claude` process is running inside `workspace` — stop the session first. `down`, `reset` and `attach-prep` always remove the engine and its network too, even if a session only ever used plain `up` (G4-41-08).
...
| Visual Studio 2026 | CLI |
| --- | --- |
| — (terminal only) | `docker compose -f .devcontainer\compose.yaml -f .devcontainer\compose.docker.yaml -p decisya-sandbox exec docker podman system df` (while the engine is up) |
...
| Inside the session: `dotnet test --no-build --filter-not-trait "Category=Integration"` | Exits `0`, 76/76 passed. `tests/Decisya.SharedKernel.Tests/RepoPaths.cs` reads the `Decisya.RepoRoot` assembly metadata `Directory.Build.props` stamps on test projects. |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `sandbox.py claude` (or the raw `docker compose exec` command) refuses to start with: *"This machine's managed settings require a first-party login, but an Anthropic-issued credential (ANTHROPIC_API_KEY…) is configured. A non-OAuth Anthropic credential cannot satisfy the org pin."* | `/etc/claude-code/managed-settings.json` had `"forceLoginMethod": "console"` baked into an earlier image. In Claude Code 2.1.273, that key pins an interactive OAuth Console login and rejects any API-key credential — the opposite of what it was added for (G4-11: a dedicated, spend-capped API key, never an interactive login). Fixed: the key has been removed from `managed-settings.json`. | Rebuild the image so the fix takes effect: `& $env:DECISYA_PYTHON .devcontainer\sandbox.py up` (rebuilds `workspace`). |
| `sandbox.py claude` or `sandbox.py shell` refuses to start with: *"refusing to start: /home/vscode/.claude/.credentials.json exists inside workspace"* | A claude.ai subscription login completed inside the sandbox at some point (N-01, `docs/security/reviews/36.md`) — `platform.claude.com`, allow-listed for the required connectivity check, also serves the OAuth endpoints. This is the intended refusal, not a bug. | `& $env:DECISYA_PYTHON .devcontainer\sandbox.py reset` (removes the home volume and the credentials file with it), then start again and use only the API key — never `/login`. |
...
- **Upload-capable allow-listed hosts.** `api.nuget.org` (with an attacker's own NuGet API key), `api.anthropic.com` and `platform.claude.com` (both with an attacker's own Anthropic account or OAuth session) can all receive data if an agent is tricked into sending it there. `platform.claude.com` adds no new *class* of exfiltration beyond what `api.anthropic.com` already allowed (same vendor); its real risk is the login path above, not data egress. The allow-list is intentionally short (no GitHub, no npm registry, no container registries) to shrink this.
